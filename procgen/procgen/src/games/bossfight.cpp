...
//Tim consts
const float INITIAL_SURVIVAL_REWARD = 0.001;  // Initial survival reward per timestep
const int MAX_TIMESTEPS = 4000;              // Maximum number of timesteps for the episode
const int HALFWAY_POINT = MAX_TIMESTEPS / 2; // Halfway point where reward hits zero
const float SURVIVAL_REWARD_DECAY = INITIAL_SURVIVAL_REWARD / HALFWAY_POINT;  // Decay rate for the survival reward
const float DEATH_PENALTY = -0.1;    // Penalty for dying or failing the episode
...
//         }
        ...
        //new Tim code:
        // Check for collisions that result in the agent's death
        if (obj->type == BOSS || obj->type == BARRIER || obj->type == ENEMY_BULLET) {
            step_data.done = true;  // End the episode due to collision
    
            // Apply the death penalty when the agent dies
    ...
    void game_step() override {
        ...
        //Tim code
        // Calculate the current survival reward
        float current_survival_reward = INITIAL_SURVIVAL_REWARD - (SURVIVAL_REWARD_DECAY * cur_time);
    
        // Apply the decayed survival reward if within the halfway point
        if (cur_time <= HALFWAY_POINT) {
            step_data.reward += current_survival_reward;  // Add survival reward for each timestep
        }
        ...
        //Tim code
        //         std::cout << "Boss Position: (" << boss->x << ", " << boss->y << ")" << std::endl;
        std::ofstream file;
        file.open("C:/Library/hailmary/boss_position.txt", std::ios_base::trunc);  // Open the file in append mode
        file << boss->x << "," << boss->y;
        file.close();
        file.open("C:/Library/hailmary/agent_position.txt", std::ios_base::trunc);  // Open the file in append mode
        file << agent->x << "," << agent->y;
        file.close();
        
        file.open("C:/Library/hailmary/bullet_positions.txt", std::ios_base::trunc); 
        for (const auto& entity : entities) {
            if (entity->type == ENEMY_BULLET) {  // Check if the entity is a bullet
                file << entity->x << "," << entity->y << ";";
            }
        }
        file.close();
        
        file.open("C:/Library/hailmary/rock_positions.txt", std::ios_base::trunc); 
        for (const auto& entity : entities) {
            if (entity->type == BARRIER) {  // Check if the entity is a bullet
                file << entity->x << "," << entity->y << ";";
            }
        }
        file.close();
